...
render_pipeline_get_bind_group_layout :: proc "contextless" (
	...
	bind_group_layout.ptr = wgpu.render_pipeline_get_bind_group_layout(ptr, index)
...
// set debug label.
render_pipeline_set_label :: proc "contextless" (using self: Render_Pipeline, label: cstring) {
	wgpu.render_pipeline_set_label(ptr, label)
}

// Increase the reference count.
render_pipeline_reference :: proc "contextless" (using self: Render_Pipeline) {
	wgpu.render_pipeline_reference(ptr)
}

// Release the `Render_Pipeline`.
render_pipeline_release :: #force_inline proc "contextless" (using self: Render_Pipeline) {
	wgpu.render_pipeline_release(ptr)
}

// Release the `Render_Pipeline` and modify the raw pointer to `nil`.
render_pipeline_release_and_nil :: proc "contextless" (using self: ^Render_Pipeline) {
