...
*/
@(require_results)
AdapterRequestDeviceSync :: proc "c" (
	self: Adapter,
	...
	if !desc_ok {
		return _adapter_request_device_sync(self, nil)
	}
	...
	return _adapter_request_device_sync(self, &raw_desc)
}

@(private)
_adapter_request_device_sync :: proc "c" (
	self: Adapter,
...
AdapterInfoString :: proc(
	...
	device_name := info.device if info.device != "" else "Unknown device"
