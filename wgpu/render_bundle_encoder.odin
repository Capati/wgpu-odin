...
Data size must be a multiple of `4` and must have an alignment of `4`. For example, with an offset
of `4` and an array of `[8]u8`, that will write to the range of `4..12`.

For each byte in the range of push constant data written, the union of the stages of all push
constant ranges that covers that byte must be exactly `stages`. There's no good way of explaining
this simply, so here are some examples:
...
You would need to upload this in two `render_bundle_encoder_set_push_constants` calls. First for the
`Vertex` range, second for the `Fragment` range.
...
You would need to upload this in three `render_bundle_encoder_set_push_constants` calls. First for
the `Vertex` only range `0..4`, second for the `{.Vertex, .Fragment}` range `4..8`, third for the
`Fragment` range `8..12`.
*/
render_bundle_encoder_set_push_constants :: proc "contextless" (
	self: Render_Bundle_Encoder,
	stages: Shader_Stage,
	offset: u32,
	data: []byte,
) {
	wgpuRenderBundleEncoderSetPushConstants(self, stages, offset, u32(len(data)), raw_data(data))
}
