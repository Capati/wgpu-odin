...
queue_submit :: proc "contextless" (self: Queue, commands: ..CommandBuffer) {
	wgpuQueueSubmit(self, uint(len(commands)), raw_data(commands))
}

queue_submit_for_index :: proc "contextless" (
	self: Queue,
	commands: ..CommandBuffer,
) -> SubmissionIndex {
	return wgpuQueueSubmitForIndex(self, uint(len(commands)), raw_data(commands))
}
...
queue_write_buffer_bytes :: proc "contextless" (
	...
	error_reset_data(loc)
	if len(data) == 0 {
		wgpuQueueWriteBuffer(self, buffer, offset, nil, 0)
	} else {
...
queue_write_buffer_raw :: proc "contextless" (
	...
	if size == 0 {
		...
		wgpuQueueWriteBuffer(self, buffer, offset, data, size)
	}
	return has_no_error()
}
...
queue_write_texture :: proc "contextless" (
	...
	error_reset_data(loc)
	destination, data_layout, write_size := destination, data_layout, write_size
	if len(data) == 0 {
		wgpuQueueWriteTexture(self, destination, nil, 0, data_layout, write_size)
	} else {
