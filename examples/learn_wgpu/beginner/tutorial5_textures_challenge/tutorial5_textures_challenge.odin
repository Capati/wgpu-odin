...
init :: proc() -> (state: ^State, err: Error) {
	...
	SHADER_SRC: string : #load("./../tutorial5_textures/shader.wgsl", string)
...
render :: proc(using state: ^State) -> (err: Error) {
	...
	if is_space_pressed {
		wgpu.render_pass_set_bind_group(render_pass, 0, cartoon_bind_group.ptr)
	} else {
		wgpu.render_pass_set_bind_group(render_pass, 0, diffuse_bind_group.ptr)
	}
