...
create :: proc() -> (self: ^Application) {
	...
	defer app.texture_release(cartoon_texture)

	self.cartoon_bind_group = wgpu.DeviceCreateBindGroup(
		self.gpu.device,
		wgpu.BindGroupDescriptor {
	...
	SHADER_WGSL :: #load("./../tutorial5_textures/shader.wgsl", string)
...
release :: proc(self: ^Application) {
	...
	wgpu.Release(self.cartoon_bind_group)
...
draw :: proc(self: ^Application) {
	...
	if app.key_is_down(self, .Space) {
		wgpu.RenderPassSetBindGroup(rpass, 0, self.cartoon_bind_group)
	} else {
