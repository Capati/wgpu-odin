...
CLIENT_WIDTH       :: 640
CLIENT_HEIGHT      :: 480
EXAMPLE_TITLE      :: "Tutorial 4 - Buffers Challenge"
...
init :: proc(self: ^Application) -> (ok: bool) {
	...
	self.num_challenge_indices = cast(u32)len(challenge_indices)

	self.challenge_vertex_buffer = wgpu.DeviceCreateBufferWithData(
		self.gpu.device,
		wgpu.BufferDataDescriptor {
			label = EXAMPLE_TITLE + " Vertex Buffer",
			contents = wgpu.ToBytes(challenge_verts[:]),
			usage = {.Vertex},
		},
	)

	self.challenge_index_buffer = wgpu.DeviceCreateBufferWithData(
		self.gpu.device,
		wgpu.BufferDataDescriptor {
			label = EXAMPLE_TITLE + " Index Buffer",
			contents = wgpu.ToBytes(challenge_indices[:]),
			usage = {.Index},
		},
	)
...
step :: proc(self: ^Application, dt: f32) -> (ok: bool) {
	...
	if app.key_is_down(.Space) {
		wgpu.RenderPassSetVertexBuffer(rpass, 0, {buffer = self.challenge_vertex_buffer})
		wgpu.RenderPassSetIndexBuffer(
			rpass,
			{buffer = self.challenge_index_buffer},
			.Uint16,
		)
		wgpu.RenderPassDrawIndexed(rpass, {0, self.num_challenge_indices})
	} else {
...
quit :: proc(self: ^Application) {
	wgpu.Release(self.challenge_index_buffer)
	wgpu.Release(self.challenge_vertex_buffer)
